...
- **Network SDR** — reads a **live IQ stream** from a receiver on your network — **SpyServer** or **rtl_tcp**
  (from 0.8.6), or **SDRConnect** (from 0.8.9) — narrows it, and streams it to RDS Bridge, which decodes it and
  plays the audio. **No other decoder needed.** For SpyServer/rtl_tcp the reason this program exists is that a
  browser can't open a raw TCP socket; for **SDRConnect** the reason is Wi-Fi — a browser can't buffer a live
  IQ stream against network jitter, so a direct browser-to-SDRConnect link over Wi-Fi stutters, and this native,
  buffered helper doesn't.
...
> ### ⚠ Keep the helper and RDS Bridge in step
...
> decodes nonsense, check this first. The **0.8.9 helper** did not change the wire format, so it works with
> **RDS Bridge 0.8.6-beta and later** (including 0.8.8); the 0.8.9 release is the helper only, with no new
> Bridge. When in doubt, download both from the same release.
...
> **Tested on:** an Apple-silicon Mac (the `.app`) with SDR++, with SpyServer over the LAN, and with
> **SDRConnect driving an RSPdxR2** over the LAN; and Windows with SDR Console. The Linux and Intel-Mac builds
> are provided and expected to work, but are not verified.
>
...
|---|---|
| Windows (most PCs) | `rds-bridge-helper-0.8.9-beta-windows-amd64.exe` |
| Mac (Apple silicon **or** Intel) | `RDS-Bridge-Helper-0.8.9-beta-macos-app.zip` |
| Linux | `rds-bridge-helper-0.8.9-beta-linux-amd64` |
...
1. Double-click `RDS-Bridge-Helper-0.8.9-beta-macos-app.zip` in Finder to unzip it — you get
...
> binary. First time only: `chmod +x rds-bridge-helper-0.8.9-beta-darwin-arm64` then
> `xattr -d com.apple.quarantine rds-bridge-helper-0.8.9-beta-darwin-arm64`. To run:
> `./rds-bridge-helper-0.8.9-beta-darwin-arm64`. (On an Intel Mac use the `darwin-amd64` filename.)

### Linux

```
chmod +x rds-bridge-helper-0.8.9-beta-linux-amd64      # first time only
./rds-bridge-helper-0.8.9-beta-linux-amd64
```
...
- **Network SDR — SDRConnect (SDRplay)** — your SDRConnect address (default `localhost:5454`). Make sure
  SDRConnect is running with a device started. **Run this helper on the same computer as SDRConnect for the
  smoothest result** — then SDRConnect feeds the helper locally at full rate, and only the narrowed stream
  crosses your network to RDS Bridge. The helper asks SDRConnect for a ~250 kHz sample rate so the network
  carries an RDS-appropriate stream, not the full device rate.

Press **Apply**. Then, in **RDS Bridge**, choose the **Network SDR** source and connect to the `ws://…`
address shown on the page (there's a **Copy** button). Tune from RDS Bridge — the helper follows. With
**SpyServer** you also get the wideband RF waterfall; **SDRConnect** does not paint the band-view waterfall
yet (the baseband display still works) — if you'd find it useful, let us know and it goes on the backlog.
You still run your SDR server (rtl_tcp / SpyServer / SDRConnect) yourself; the helper connects *out* to it,
and it can be on this machine or another box on your network.
...
- `-sdrconnect addr` — SDRConnect WebSocket source, e.g. `-sdrconnect 192.168.1.10:5454` (Network SDR mode).
